...
本仓库提供 `install_sdk.sh`，用于在 Debian/Ubuntu 系统上一键安装
Livox-SDK2。脚本会安装必要的编译依赖、以 Release 模式编译 SDK、安装头文件和
库，并刷新系统动态库缓存。
...
脚本可以重复执行。可通过环境变量调整安装行为：

```bash
# 设置并行编译线程数
LIVOX_BUILD_JOBS=8 ./install_sdk.sh

# 已经安装依赖时跳过 apt
LIVOX_SKIP_APT=1 ./install_sdk.sh

# 修改安装前缀
LIVOX_INSTALL_PREFIX=/usr/local ./install_sdk.sh
```

非 root 用户需要能够使用 `sudo`。安装完成后，可以检查动态库是否已经被系统识别：

```bash
ldconfig -p | grep liblivox_lidar_sdk
```

Ubuntu 24.04 使用 GCC 13；本仓库已经补充上游源码遗漏的 `<cstdint>` 标准头文件，
并已在 Ubuntu 24.04、ROS 2 Jazzy 环境完成实际编译验证。
